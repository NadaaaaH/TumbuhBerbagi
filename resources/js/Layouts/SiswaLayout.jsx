...
export default function SiswaLayout({ user, header, children, examMode = false }) {
    ...
    const isExam = examMode || /^\/(latihan|tryout)\/\d+(\?.*)?$/.test(url);
    ...
    return (
        <div className="min-h-screen bg-[#FAFBFC] font-['Inter',sans-serif] flex">
            ...
            {!isExam && (
                <aside
                    className="hidden md:flex flex-col w-64 bg-gradient-to-b from-[#1b5e20] to-[#124216] text-white border border-emerald-800/50 fixed right-4 top-4 z-30 shrink-0 shadow-[0_12px_40px_rgba(0,0,0,0.15)] rounded-[2.2rem]"
            ...
            {/* Mobile Navigation Bar (Hidden during exam) */}
            {!isExam && (
            ...
            {/* Main Content Area */}
            <div className={`flex-1 flex flex-col min-h-screen min-w-0 ${!isExam ? 'overflow-x-hidden pt-16 md:pt-0' : ''}`}>

                {/* Exam Compact Floating Capsule Navbar (Landing Page style with Framer Motion) */}
                {isExam && (
                    <div className={`fixed w-full z-50 pointer-events-none transition-all duration-500 ${scrolled ? 'top-3 px-3 sm:px-6 md:px-8' : 'top-4 px-4 sm:px-6 md:px-8'}`}>
                        <motion.header
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3 sm:gap-4 shrink-0">
                                <Link href="/" className="shrink-0 flex items-center group">
                                    <img
                                        src="/images/logo.png"
                                        alt="Tumbuh Berbagi"
                                        className="h-8 sm:h-9 w-auto shrink-0 group-hover:scale-105 transition-transform duration-300"
                                    />
                                </Link>
                                ...
                                >
                                    <PrimaryButton className="px-4 sm:px-5 py-2 text-xs gap-2 shrink-0 shadow-md hover:shadow-[0_4px_20px_rgba(27,94,32,0.3)] active:scale-95 transition-all duration-300">
                                        ...
                                        <span className="hidden sm:inline">
                                            {url.startsWith('/tryout') ? 'Kembali ke Try Out' : 'Kembali ke Daftar Latihan'}
                                        </span>
                            ...
                            <div className="min-w-0 px-2 sm:px-4 text-center flex-1 max-w-lg mx-auto">
                                <h2 className="text-base sm:text-lg md:text-xl font-extrabold text-slate-800 truncate font-['Poppins'] tracking-tight">
                                    {header}
                                </h2>
                            </div>
                ...
                {/* Mobile Header Title (Only for standard non-exam mode) */}
                {!isExam && (
                ...
                {/* Main Content */}
                <main className={`flex-1 p-4 md:p-8 relative ${isExam ? 'w-full pt-24 md:pt-28' : 'md:mr-72'}`}>
                ...
                {/* Footer Section (Hidden during exam) */}
                {!isExam && (
